...
def main():
    ...
    parser.add_argument("command", choices=["run", "update", "export", "prices"],
                        help="run/resume a frozen range; update appends finalized blocks and prices; export is offline")
    parser.add_argument("--start-block", type=int, default=7_000_000, help="Start block for run; update uses the saved journal")
    ...
    if args.command in ("run", "update", "prices") and not url:
    ...
    if args.command == "run":
        ...
        pipeline.update(args.end_block)
